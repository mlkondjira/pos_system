...
class CashSessionHistoryScreen extends StatefulWidget {
  ...
  @override
  State<CashSessionHistoryScreen> createState() => _CashSessionHistoryScreenState();
}
...
class _CashSessionHistoryScreenState extends State<CashSessionHistoryScreen> {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => CashHistoryBloc(getIt<PosDatabase>())..add(LoadCashHistory()),
      child: Scaffold(
          ...
          actions: [
            BlocBuilder<CashHistoryBloc, CashHistoryState>(
              builder: (context, state) {
                return IconButton(
                  icon: Icon(Icons.calendar_month_outlined, color: state.dateRange != null ? AppColors.accent : null),
                  ...
                  child: Row(
                    children: [
                      ...
                      IconButton(
                        ...
                        onPressed: () => context.read<CashHistoryBloc>().add(const DateRangeChanged(null)),
              ...
              child: BlocBuilder<CashHistoryBloc, CashHistoryState>(
                builder: (context, state) {
                  ...
                  if (state.sessions.isEmpty) {
                    return Center(
                      child: Text(state.dateRange == null
                  ...
                  return ListView.separated(
                    ...
                    itemBuilder: (context, index) {
                      return _SessionTile(
                        ...
                        onExportPdf: () => _exportPdf(state.sessions[index]),
                      );
                    },
                  );
                },
  ...
  void _printReport(CashSessionWithUser sessionData) async {
    ...
    try {
      ...
      final payments = await db.salesDao.getPaymentsForSession(sessionData.session.id);
      ...
      messenger.showSnackBar(const SnackBar(content: Text('Rapport envoyé à l\'imprimante.'), backgroundColor: AppColors.success));
    } catch (e) {
      if (!mounted) return;
      navigator.pop(); // close loading
      messenger.showSnackBar(SnackBar(content: Text('Erreur d\'impression: $e'), backgroundColor: AppColors.danger));
    }
  }
  ...
  void _exportPdf(CashSessionWithUser sessionData) async {
    ...
    try {
        ...
        build: (pw.Context context) {
          return pw.Column(
            ...
            children: [
              ...
              pw.Center(
                child: pw.Text(shopName.toUpperCase(), style: pw.TextStyle(fontWeight: pw.FontWeight.bold, fontSize: 24)),
              ),
              pw.SizedBox(height: 20),
              pw.Center(child: pw.Text('RAPPORT DE CLÔTURE DE CAISSE', style: pw.TextStyle(fontSize: 18, fontWeight: pw.FontWeight.bold))),
              pw.SizedBox(height: 20),
              ...
              pw.Row(mainAxisAlignment: pw.MainAxisAlignment.spaceBetween, children: [
                pw.Text('Ouverture: ${Fmt.time(sessionData.session.startedAt)}'),
                if (sessionData.session.endedAt != null)
                  pw.Text('Fermeture: ${Fmt.time(sessionData.session.endedAt!)}'),
              ]),
              pw.Divider(thickness: 1.5),

              pw.SizedBox(height: 10),
              pw.Text('DÉTAIL', style: pw.TextStyle(fontWeight: pw.FontWeight.bold)),
              pw.SizedBox(height: 10),

              _pdfRow('Fond de caisse', sessionData.session.startingCash, currency),
              _pdfRow('Ventes espèces', cashSales, currency),
              pw.Divider(),
              _pdfRow('TOTAL ESPÈCES ATTENDU', sessionData.session.expectedCash ?? 0, currency, isBold: true),

              pw.SizedBox(height: 20),
              pw.Text('COMPTAGE', style: pw.TextStyle(fontWeight: pw.FontWeight.bold)),
              pw.SizedBox(height: 10),

              _pdfRow('Montant compté', sessionData.session.endingCash ?? 0, currency),
              _pdfRow('Écart', sessionData.session.discrepancy ?? 0, currency,
                  color: (sessionData.session.discrepancy ?? 0) != 0 ? PdfColors.red : null),

              if (otherPayments.isNotEmpty) ...[
                pw.SizedBox(height: 20),
                pw.Text('AUTRES PAIEMENTS', style: pw.TextStyle(fontWeight: pw.FontWeight.bold)),
                pw.SizedBox(height: 10),
                ...otherPayments.entries.map((e) => _pdfRow(Fmt.paymentMethod(e.key), e.value, currency)),
              ],
      ...
      if (!mounted) return;
      navigator.pop();
      messenger.showSnackBar(SnackBar(content: Text('Erreur export PDF: $e'), backgroundColor: AppColors.danger));
    }
  }

  pw.Widget _pdfRow(String label, double value, String currency, {bool isBold = false, PdfColor? color}) {
    return pw.Padding(
      ...
      child: pw.Row(
        ...
        children: [
          ...
          pw.Text(Fmt.currency(value, symbol: currency), style: pw.TextStyle(fontWeight: isBold ? pw.FontWeight.bold : null, color: color)),
        ],
      ),
    );
  }
...
class _SessionTile extends StatelessWidget {
  ...
  const _SessionTile({required this.sessionData, required this.onPrint, required this.onExportPdf});
