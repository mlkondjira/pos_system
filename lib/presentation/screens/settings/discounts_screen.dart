...
class _DiscountsScreenState extends State<DiscountsScreen> {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          title: const Text('Gestion des Remises', style: TextStyle(fontWeight: FontWeight.bold)),
          elevation: 0,
          actions: [
            ...
            BlocBuilder<DiscountsBloc, DiscountsState>(
              builder: (context, state) {
                ...
                final hasExpired = state.discounts.any((d) => !d.isArchived && d.endDate != null && now.isAfter(d.endDate!));
        ...
        body: BlocBuilder<DiscountsBloc, DiscountsState>(
          builder: (context, state) {
            ...
            final activeCount = state.discounts.where((d) => !d.isArchived && (d.endDate == null || !now.isAfter(d.endDate!))).length;
            final expiredCount = state.discounts.where((d) => !d.isArchived && d.endDate != null && now.isAfter(d.endDate!)).length;
            final archivedCount = state.discounts.where((d) => d.isArchived).length;
            
            // 1. Filtrage par statut
            var filteredDiscounts = state.discounts.where((d) {
              if (_selectedFilter == 'archived') return d.isArchived;
              if (d.isArchived) return false; // Ne pas montrer les archivés dans les autres onglets

              final bool isExpired = d.endDate != null && now.isAfter(d.endDate!);
            ...
            return Column(
              children: [
                ...
                _buildFilterChips(active: activeCount, expired: expiredCount, archived: archivedCount),
                Expanded(
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final wasNotifiedToday = state.notifiedDates[d.id] == today;
                        ...
                        child: Opacity(
                          ...
                          child: ListTile(
                            ...
                            leading: CircleAvatar(
                              backgroundColor: (d.isActive && !isExpired) ? AppColors.successSoft : AppColors.bg,
                              child: Icon(
                                d.type == 'percentage' ? Icons.percent_rounded : Icons.money_rounded,
                                color: (d.isActive && !isExpired) ? AppColors.success : AppColors.textMuted,
                              ),
                            ),
                            title: Row(
                              children: [
                                ...
                                if (isExpired) ...[
                                  ...
                                  const StatusBadge(label: 'EXPIRÉ', color: AppColors.danger),
                                ] else if (wasNotifiedToday) ...[
                                  const SizedBox(width: 8),
                                  const Tooltip(
                                    message: 'Une alerte d\'expiration a été envoyée aujourd\'hui pour cette promotion.',
                                    child: Icon(Icons.notification_important_rounded, color: AppColors.warning, size: 18),
                                  ),
                                ],
                              ],
                            ),
                            subtitle: Column(
                              ...
                              children: [
                                ...
                                Text('Min. achat: ${Fmt.currency(d.minAmount)}', style: const TextStyle(fontSize: 12)),
                                Text(
                                  'Usage: ${d.currentUsage}${d.usageLimit != null ? " / ${d.usageLimit}" : ""}', 
                                  style: TextStyle(fontSize: 12, color: (d.usageLimit != null && d.currentUsage >= d.usageLimit!) ? AppColors.danger : AppColors.textSecondary),
                                ),
                                if (d.endDate != null)
                                  Padding(
                            ...
                            trailing: Row(
                              ...
                              if (d.isArchived)
                                IconButton(
                                  ...
                                  tooltip: 'Restaurer',
                                  onPressed: () => _confirmRestore(context, d),
                                ),
                                IconButton(
  ...
  Future<void> _generateMonthlyReport(BuildContext context) async {
    ...
    if (stats.isEmpty) {
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Aucun coupon utilisé ce mois-ci.'), backgroundColor: AppColors.info),
        );
      }
      return;
    }
    ...
    await getIt<PrinterService>().sharePdfReport(
      ...
      shareMessage: 'Voici le rapport d\'utilisation des coupons pour le mois de $monthName.',
      subject: 'Rapport Coupons POS - $monthName',
      tableHeaders: ['Code Coupon', 'Usage', 'Remise Totale', 'CA Généré'],
      tableData: tableData,
    );
  }
  Widget _buildFilterChips({required int active, required int expired, required int archived}) {
    return SingleChildScrollView(
      scrollDirection: Axis.horizontal,
      padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
      child: Row(
        children: [
          _filterChip('active', 'Actives', Icons.check_circle_outline_rounded, active),
          const SizedBox(width: 8),
  ...
  void _confirmArchiveAllExpired(BuildContext context) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text('Voulez-vous déplacer toutes les remises expirées vers les archives ? Elles ne seront plus utilisables en caisse.'),
        actions: [
  ...
  void _confirmResetUsage(BuildContext context, Discount d) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text('Voulez-vous remettre à zéro le nombre d\'utilisations pour "${d.name}" ?\nActuellement : ${d.currentUsage}'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('ANNULER')),
          ElevatedButton(
            onPressed: () {
  ...
  void _confirmRestore(BuildContext context, Discount d) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text('La remise "${d.name}" sera de nouveau visible et pourra être appliquée en caisse.'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('ANNULER')),
          ElevatedButton(
            onPressed: () {
  ...
  void _confirmArchive(BuildContext context, Discount d) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text('La remise "${d.name}" sera masquée de la liste et ne pourra plus être appliquée en caisse.'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('ANNULER')),
          ElevatedButton(
            onPressed: () {
  ...
  void _showForm(BuildContext context, {Discount? discount}) {
    showDialog(
      ...
      builder: (ctx) => DiscountFormDialog(
        discount: discount,
        onSave: (companion) => context.read<DiscountsBloc>().add(UpsertDiscount(companion)),
      ),
    );
  }
...
class DiscountFormDialog extends StatefulWidget {
  ...
  @override State<DiscountFormDialog> createState() => _DiscountFormDialogState();
}
...
class _DiscountFormDialogState extends State<DiscountFormDialog> {
  ...
  @override void initState() {
    ...
    _minAmountCtrl = TextEditingController(text: widget.discount?.minAmount.toString() ?? '0');
    _limitCtrl = TextEditingController(text: widget.discount?.usageLimit?.toString() ?? '');
  ...
  @override Widget build(BuildContext context) {
    return AlertDialog(
      title: Text(widget.discount == null ? 'Ajouter une remise' : 'Modifier la remise'),
      content: SingleChildScrollView(
        child: Form(
          key: _formKey,
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            TextFormField(controller: _nameCtrl, decoration: const InputDecoration(labelText: 'Code / Nom (ex: SOLDES10)'), 
                validator: (v) => v!.isEmpty ? 'Requis' : null, textCapitalization: TextCapitalization.characters),
            ...
            const SizedBox(height: 16),
            TextFormField(controller: _valueCtrl, keyboardType: const TextInputType.numberWithOptions(decimal: true), 
                decoration: InputDecoration(labelText: 'Valeur', suffixText: _type == 'percentage' ? '%' : 'FCFA'),
                validator: (v) => double.tryParse(v ?? '') == null ? 'Invalide' : null),
            const SizedBox(height: 16),
            TextFormField(controller: _minAmountCtrl, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Montant min. d\'achat')),
            const SizedBox(height: 16),
            TextFormField(controller: _limitCtrl, keyboardType: TextInputType.number, decoration: const InputDecoration(labelText: 'Limite d\'usage totale', hintText: 'Ex: 50 (Laisser vide pour illimité)')),
            SwitchListTile(title: const Text('Remise active'), value: _isActive, onChanged: (v) => setState(() => _isActive = v), activeThumbColor: AppColors.success),
            SwitchListTile(title: const Text('Une seule fois par client'), value: _limitPerCustomer, onChanged: (v) => setState(() => _limitPerCustomer = v), activeThumbColor: AppColors.primary),
            const Divider(),
            ListTile(
              contentPadding: EdgeInsets.zero,
              leading: const Icon(Icons.date_range_rounded, color: AppColors.primary),
              title: const Text('Date de début', style: TextStyle(fontSize: 13)),
              subtitle: Text(_startDate == null ? 'Immédiat' : Fmt.date(_startDate!)),
              trailing: _startDate != null ? IconButton(icon: const Icon(Icons.close, size: 18), onPressed: () => setState(() => _startDate = null)) : null,
              onTap: () => _pickDate(true),
            ),
            ListTile(
              contentPadding: EdgeInsets.zero,
              leading: const Icon(Icons.event_busy_rounded, color: AppColors.danger),
              title: const Text('Date d\'expiration', style: TextStyle(fontSize: 13)),
              subtitle: Text(_endDate == null ? 'Jamais' : Fmt.date(_endDate!)),
              trailing: _endDate != null ? IconButton(icon: const Icon(Icons.close, size: 18), onPressed: () => setState(() => _endDate = null)) : null,
      ...
      actions: [
        ...
        ElevatedButton(onPressed: () async {
          ...
          final companion = DiscountsCompanion(id: widget.discount == null ? const Value.absent() : Value(widget.discount!.id), name: Value(_nameCtrl.text.trim().toUpperCase()),
              type: Value(_type), value: Value(double.parse(_valueCtrl.text)), 
              ...
              startDate: Value(_startDate), endDate: Value(_endDate),
              shopId: Value(shopId));
          if (!mounted) return;
